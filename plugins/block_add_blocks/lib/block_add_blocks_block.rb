class BlockAddBlocksBlock < Block
  ...
  def help
    _('This block displays articles chosen by you.')
  end
  ...
  def url_params
    ...
    if self.owner.is_a?(Profile)
      params.merge!(:controller => "block_add_blocks_plugin_myprofile")
      params.merge!(:profile => self.holder.identifier)
    else
      params.merge!( :controller => "block_add_blocks_plugin_admin")
    end
    params
  end
